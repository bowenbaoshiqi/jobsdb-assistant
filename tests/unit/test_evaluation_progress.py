...
def test_append_creates_batch_and_deduplicates_tasks(tmp_path) -> None:
    store = EvaluationProgressStore(tmp_path / "progress.json")
    ...
    assert appended == 2
    assert batch.tasks == {
        "job-1": EvaluationTaskStatus.QUEUED,
        "job-2": EvaluationTaskStatus.QUEUED,
    }


def test_append_preserves_existing_statuses_and_batch_identity(
    tmp_path,
) -> None:
    store = EvaluationProgressStore(tmp_path / "progress.json")
    original = store.start(["job-1", "job-2"], now=NOW)
    store.mark("job-1", EvaluationTaskStatus.COMPLETED)
    ...
    assert batch.started_at == original.started_at
    assert batch.tasks == {
        "job-1": EvaluationTaskStatus.COMPLETED,
        "job-2": EvaluationTaskStatus.QUEUED,
        "job-3": EvaluationTaskStatus.QUEUED,
    }


def test_append_empty_input_does_not_create_batch(tmp_path) -> None:
    path = tmp_path / "progress.json"
    store = EvaluationProgressStore(path)

    batch, appended = store.append([], now=NOW)
